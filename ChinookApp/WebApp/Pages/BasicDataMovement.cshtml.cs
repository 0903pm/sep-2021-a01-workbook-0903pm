...
namespace WebApp.Pages
{
    public class BasicDataMovementModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            //when the page is retrieved in response to a form POST, the Get Request is NOT automatically requested.
        ...
        {
            //IF you do NOT use asp-page-handler on your buttons in your form, the general method=post will request the OnPost() event
            Thread.Sleep(2000);
            string buttonValue = Request.Form["theButton"];
            FeedBackMessage = buttonValue;

            //theIActionResult of RedirectToPage() sends an OnGet request to be executed after the OnPost is complete
            //without an href, the RedirectToPage defaults to the cuurent page
            return RedirectToPage();
        }

        public IActionResult OnPostAButton()
        {
            //if you are using the asp-page-handler(xxxx) then your post events will have a method name of OnPostxxxx()
            Thread.Sleep(1000);
